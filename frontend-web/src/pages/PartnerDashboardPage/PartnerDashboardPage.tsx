...
import MonthlySnapshot from '../../components/PartnerDashboard/MonthlySnapshot';
import {
  Bike as BikeIcon, // Renamed to avoid conflict with imported Bike type
  Star,
  Users,
  TrendingUp,
  Clock,
...
import { usePartnerRealtimeEvents } from '../../hooks/useRealtimeEvents';
import notificationIntegrationService from '../../services/notificationIntegrationService';
import { Bike, bikeService } from '../../services/bikeService'; // Import bikeService and Bike type
...
const PartnerDashboardPage = () => {
  ...
  const [partner, setPartner] = useState<Partner | null>(null);
  const [partnerLoading, setPartnerLoading] = useState(true);
  const [totalBikes, setTotalBikes] = useState(0); // New state for total bikes
  ...
  // New function to fetch total bikes
  const fetchTotalBikes = async () => {
    try {
      const bikeList: Bike[] = await bikeService.getMyBikes();
      setTotalBikes(bikeList.length);
    } catch (err) {
      console.error('Error fetching total bikes:', err);
  ...
  useEffect(() => {
    ...
    if (user && user.role === 'partner') {
      console.log('[PartnerDashboard] Initializing for partner user:', user.id);
      fetchBookings();
      fetchTotalBikes(); // Fetch total bikes here
    ...
    if (newBookingRequests.length > 0) {
      ...
      const refreshBookings = async () => {
        try {
          ...
          const transformedBookings = backendBookings.map(transformBookingForPartnerDashboard);
          setBookings(transformedBookings);
          fetchTotalBikes(); // Also refresh total bikes on new booking
            ...
            <div className="grid md:grid-cols-3 gap-6 mb-8">
              <div className="bg-white rounded-xl p-6 shadow-sm">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <div className="text-2xl font-bold text-gray-900">{totalBikes}</div> {/* Use totalBikes here */}
                    <div className="text-sm text-gray-600">Total Bikes</div>
                  </div>
                </div>
              </div>
